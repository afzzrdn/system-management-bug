...
const CheckIcon = () => (<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" /></svg>);
        ...
        {(form.data.attachments as (File | string)[]).map((file, index) => (
          <div key={index} className="text-sm text-gray-700 flex items-center justify-between bg-gray-100 p-2 rounded-md">
            <span className="truncate">
              {file instanceof File ? file.name : (<a href={file} target="_blank" rel="noopener noreferrer" className="text-indigo-600 hover:underline">Lihat attachment {index + 1}</a>)}
            </span>
            <button type="button" onClick={() => { const updated = [...(form.data.attachments as (File | string)[])]; updated.splice(index, 1); form.setData('attachments', updated); }} className="ml-4 flex-shrink-0 text-red-500 hover:text-red-700 font-semibold text-xs">Hapus</button>
          </div>
        ))}
      </div>
    );
              ...
              <Dialog.Panel className="w-full max-w-2xl transform overflow-hidden rounded-2xl bg-white p-8 text-left align-middle shadow-xl transition-all">
                <div className="w-full max-w-md mx-auto mb-8">
                  <div className="flex items-center">
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center text-white transition-colors duration-300 ${step >= 1 ? 'bg-indigo-600' : 'bg-gray-300'}`}>{step > 1 ? <CheckIcon /> : '1'}</div>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-2">
                        ...
                        <div className="md:col-span-1">
                          ...
                          <input type="datetime-local" value={(form.data.due_at as any) || ''} onChange={e => form.setData('due_at', e.target.value || null)} className="mt-2 p-3 block w-full rounded-md border-gray-300 shadow-sm focus:outline-none sm:text-sm" />
                          {form.errors.due_at && <p className="text-red-500 text-xs mt-1">{form.errors.due_at}</p>}
                        </div>
                      </div>
                    </div>
                  )}
                        ...
                        <div className="text-center">
                          ...
                          <div className="mt-4 flex justify-center text-sm leading-6 text-gray-600">
                            <label htmlFor="attachment-upload" className="relative cursor-pointer rounded-md bg-white font-semibold text-indigo-600 focus-within:outline-none focus-within:ring-2 focus-within:ring-indigo-600 focus-within:ring-offset-2 hover:text-indigo-500">
                              ...
                              <input id="attachment-upload" name="attachment" type="file" className="sr-only" multiple onChange={e => { const files = e.target.files ? Array.from(e.target.files) : []; const existing = Array.isArray(form.data.attachments) ? form.data.attachments : []; form.setData('attachments', [...existing, ...files]); }} />
                            </label>
